...
import { BookOpen, Plus, Trash2, ChevronLeft, ChevronRight, Lock, X, ScrollText, Menu, LogOut, Pencil, Search, Star, Bookmark, BookmarkCheck, Share2, Eye, ArrowUp, ArrowDown, MessageCircle, Upload, Download, Sliders, Maximize, Minimize, Check } from "lucide-react";
...
const LS_READER_SETTINGS = "scroll_reader_settings";
const LS_READ_CHAPTERS = "scroll_read_chapters";

function getReaderSettings() {
  try {
    const s = JSON.parse(localStorage.getItem(LS_READER_SETTINGS));
    return { brightness: 0, width: 100, theme: "dark", ...(s || {}) };
  } catch { return { brightness: 0, width: 100, theme: "dark" }; }
}
function saveReaderSettings(next) {
  try { localStorage.setItem(LS_READER_SETTINGS, JSON.stringify(next)); } catch {}
}
function getReadChapters() {
  try { return JSON.parse(localStorage.getItem(LS_READ_CHAPTERS)) || []; } catch { return []; }
}
function markChapterRead(chapterId) {
  try {
    const cur = getReadChapters();
    if (!cur.includes(chapterId)) {
      localStorage.setItem(LS_READ_CHAPTERS, JSON.stringify([...cur, chapterId]));
    }
  } catch {}
}

// --- scheduled publishing: a chapter is visible once published AND its scheduled time (if any) has passed ---
function isChapterVisible(c) {
  if (!c || c.status === "draft") return false;
  if (c.publishAt && c.publishAt > Date.now()) return false;
  return true;
}

const READER_THEMES = {
  dark: { bg: "#14131b", fg: "#ede8dd", surface: "#1e1d29", hairline: "rgba(237,232,221,0.1)", muted: "#8b879a" },
  light: { bg: "#f7f5f0", fg: "#1a1a1a", surface: "#ffffff", hairline: "rgba(0,0,0,0.12)", muted: "#6b6b6b" },
  sepia: { bg: "#f4ecd8", fg: "#3a2f1f", surface: "#ece0c5", hairline: "rgba(58,47,31,0.16)", muted: "#8a7a5c" },
};
...
function referrerSource() {
  ...
  try {
    ...
    if (/discord/i.test(host)) return "discord";
...
async function trackView(field, id) {
  ...
  try {
    await updateDoc(SITE_DOC, {
      [`analytics.${field}.${id}`]: increment(1),
      [`analytics.dailyViews.${todayKey()}`]: increment(1),
    });
  } catch (e) {
...
async function trackReferrer() {
  try {
    await updateDoc(SITE_DOC, { [`analytics.referrers.${todayKey()}.${referrerSource()}`]: increment(1) });
  } catch (e) {
    // non-critical
  }
}
function pruneOldDaily(dailyObj, days = 30) {
  if (!dailyObj) return dailyObj;
  const cutoff = Date.now() - days * 86400000;
  const out = {};
  for (const [date, val] of Object.entries(dailyObj)) {
    const t = new Date(date + "T00:00:00Z").getTime();
    if (!isNaN(t) && t >= cutoff) out[date] = val;
  }
  return out;
}
...
export default function WebtoonSite() {
  ...
  useEffect(() => {
    try {
      if (!sessionStorage.getItem("scroll_ref_tracked")) {
        sessionStorage.setItem("scroll_ref_tracked", "1");
        trackReferrer();
      }
    } catch {}
  }, []);
  ...
  useEffect(() => {
    ...
    const siteName = settings.siteTitle?.trim() || "SCROLL.";
    const defaultDesc = settings.tagline?.trim() || "Vertical stories, one panel at a time.";
    let pageTitle = `${siteName} — ${defaultDesc}`;
    let description = defaultDesc;
    let image = settings.logoUrl?.trim() || "";

    if (view === "series" && seriesId) {
      const s = data.series?.find(x => x.id === seriesId);
      if (s) {
        pageTitle = `${s.title} — ${siteName}`;
        description = s.description?.trim().slice(0, 160) || `Read ${s.title} online on ${siteName}.`;
        image = s.coverUrl || image;
      }
    } else if (view === "reader" && seriesId) {
      const s = data.series?.find(x => x.id === seriesId);
      const c = data.chapters?.find(x => x.id === chapterId);
      if (s) {
        pageTitle = c ? `${s.title} — Ep. ${String(c.number).padStart(2, "0")} | ${siteName}` : `${s.title} — ${siteName}`;
        description = c ? `Read ${s.title} episode ${c.number} online on ${siteName}.` : `Read ${s.title} online on ${siteName}.`;
        image = s.coverUrl || image;
      }
    }
    ...
    setMeta("property", "og:type", view === "home" ? "website" : "article");
  ...
  return (
    <div style={{ background: "var(--ink)", color: "var(--paper)", minHeight: "100vh", fontFamily: "'Noto Sans KR', sans-serif" }}>
      ...
      {!loaded ? (
        <div className="flex items-center justify-center" style={{ height: "100vh", flexDirection: "column", gap: 14 }}>
          <ScrollText size={30} color="var(--accent)" />
          <span className="mono" style={{ color: "var(--muted)", fontSize: 13 }}>Loading…</span>
        </div>
      ) : (
        <>
          {view !== "reader" && (
...
function Home({ data, onOpenSeries, onGoAdmin, onOpenChapter }) {
  ...
  const newSeriesIds = new Set(
    data.chapters
      .filter(c => isChapterVisible(c) && c.createdAt && now - c.createdAt < NEW_WINDOW_MS)
      .map(c => c.seriesId)
  );
  ...
  const latestChapters = data.chapters
    .filter(c => isChapterVisible(c) && visibleSeries.some(s => s.id === c.seriesId))
          ...
          {filteredSeries.length === 0 ? (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))", gap: 24 }}>
              {filteredSeries.map(s => (
                <SeriesCard key={s.id} series={s} chapterCount={data.chapters.filter(c => c.seriesId === s.id && isChapterVisible(c)).length} isNew={newSeriesIds.has(s.id)} onClick={() => onOpenSeries(s.id)} />
              ))}
            </div>
          )}
        </>
...
function SeriesPage({ data, seriesId, onOpenChapter, onBack, addComment }) {
  ...
  const chapters = data.chapters.filter(c => c.seriesId === seriesId && isChapterVisible(c)).sort((a, b) => a.number - b.number);
  const [bookmarked, setBookmarked] = useState(() => isBookmarked(seriesId));
  const [shared, setShared] = useState(false);
  const [name, setName] = useState("");
  const [text, setText] = useState("");
  const [rating, setRating] = useState(0);
  const [readChapters] = useState(() => new Set(getReadChapters()));
  ...
  const share = async () => {
    const url = `${window.location.origin}/api/share?type=series&id=${encodeURIComponent(seriesId)}`;
    if (navigator.share) {
        ...
        <div style={{ borderTop: "1px solid var(--hairline)" }}>
          {chapters.map((c) => {
            ...
            <button key={c.id} onClick={() => onOpenChapter(series.id, c.id)} className="hairline"
              style={{ width: "100%", display: "flex", alignItems: "center", gap: 18, padding: "18px 4px", borderBottom: "1px solid", background: "none", cursor: "pointer", color: "var(--paper)", textAlign: "left", opacity: isRead ? 0.55 : 1 }}>
              <span className="mono" style={{ fontSize: 20, color: isRead ? "var(--muted)" : "var(--accent)", width: 56, flexShrink: 0 }}>{String(c.number).padStart(2, "0")}</span>
              <span style={{ flex: 1, fontSize: 15, fontWeight: 500 }}>{c.title}</span>
              {isRead && <Check size={14} color="var(--jade)" />}
...
function Reader({ data, seriesId, chapterId, onOpenChapter, onBackToSeries }) {
  ...
  const toggleFullscreen = async () => {
    ...
    try {
      if (!document.fullscreenElement) {
        ...
        if (screen.orientation?.lock) { try { await screen.orientation.lock("portrait"); } catch {} }
      } else {
  ...
  const share = async () => {
    const url = `${window.location.origin}/api/share?type=chapter&seriesId=${encodeURIComponent(seriesId)}&chapterId=${encodeURIComponent(chapterId)}`;
    if (navigator.share) {
  ...
  return (
    <div ref={containerRef} style={{
      position: "relative", background: (READER_THEMES[readerSettings.theme] || READER_THEMES.dark).bg, minHeight: "100vh",
      "--ink": (READER_THEMES[readerSettings.theme] || READER_THEMES.dark).bg,
      "--paper": (READER_THEMES[readerSettings.theme] || READER_THEMES.dark).fg,
      "--surface": (READER_THEMES[readerSettings.theme] || READER_THEMES.dark).surface,
      "--hairline": (READER_THEMES[readerSettings.theme] || READER_THEMES.dark).hairline,
      "--muted": (READER_THEMES[readerSettings.theme] || READER_THEMES.dark).muted,
    }}>
      {/* progress rail */}
      ...
      <div className="flex items-center justify-between" style={{ maxWidth: 780, margin: "0 auto", padding: "24px 24px 0" }}>
        ...
        <div className="flex items-center" style={{ gap: 14, position: "relative" }}>
          <button onClick={toggleFullscreen} className="mono" style={{
            background: "none", border: "1px solid var(--hairline)", color: "var(--paper)", cursor: "pointer",
            fontSize: 11, fontWeight: 700, letterSpacing: "0.04em", padding: "6px 10px", borderRadius: 999,
          ...
          <button onClick={() => setSettingsOpen(o => !o)} aria-label="Reading settings" style={{
            background: settingsOpen ? "var(--surface)" : "none", border: "1px solid var(--hairline)", color: "var(--paper)",
            width: 30, height: 30, borderRadius: "50%", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0
          }}>
            <Sliders size={14} />
          </button>
          {settingsOpen && (
            <>
              <div onClick={() => setSettingsOpen(false)} style={{ position: "fixed", inset: 0, zIndex: 39 }} />
              <div style={{
                ...
                padding: "16px 18px", width: 220, boxShadow: "0 12px 30px rgba(0,0,0,0.4)"
              }}>
                <div style={{ marginBottom: 16 }}>
                  <span className="mono" style={{ fontSize: 11, color: "var(--muted)", textTransform: "uppercase", display: "block", marginBottom: 8 }}>Theme</span>
                  <div className="flex" style={{ gap: 6 }}>
                    {["dark", "light", "sepia"].map(t => (
                      <button key={t} onClick={() => updateReaderSetting("theme", t)} className="mono" style={{
                        flex: 1, textTransform: "capitalize", fontSize: 11, fontWeight: 700, padding: "7px 0", borderRadius: 6, cursor: "pointer",
                        border: "1px solid var(--hairline)",
                        background: readerSettings.theme === t ? "var(--accent)" : "transparent",
                        color: readerSettings.theme === t ? "var(--ink)" : "var(--paper)"
                      }}>{t}</button>
                    ))}
                  </div>
                </div>
                <div style={{ marginBottom: 16 }}>
                  <div className="flex items-center justify-between" style={{ marginBottom: 6 }}>
                    <span className="mono" style={{ fontSize: 11, color: "var(--muted)", textTransform: "uppercase" }}>Brightness</span>
                    <span className="mono" style={{ fontSize: 11, color: "var(--accent)" }}>{100 - readerSettings.brightness}%</span>
                  </div>
                  <input type="range" min={0} max={80} value={readerSettings.brightness}
                    onChange={e => updateReaderSetting("brightness", Number(e.target.value))}
                    style={{ width: "100%", accentColor: "var(--accent)" }} />
                </div>
                <div>
                  <div className="flex items-center justify-between" style={{ marginBottom: 6 }}>
                    <span className="mono" style={{ fontSize: 11, color: "var(--muted)", textTransform: "uppercase" }}>Page Width</span>
                    <span className="mono" style={{ fontSize: 11, color: "var(--accent)" }}>{readerSettings.width}%</span>
                  </div>
                  <input type="range" min={50} max={100} step={5} value={readerSettings.width}
                    onChange={e => updateReaderSetting("width", Number(e.target.value))}
                    style={{ width: "100%", accentColor: "var(--accent)" }} />
                </div>
              </div>
            </>
      ...
      {readerSettings.brightness > 0 && (
        <div style={{ position: "fixed", inset: 0, background: "#000", opacity: readerSettings.brightness / 100, pointerEvents: "none", zIndex: 21 }} />
      )}

      <h2 className="display" style={{ textAlign: "center", fontSize: 22, margin: "18px 0 30px" }}>{chapter.title}</h2>

      <div style={{ maxWidth: 720 * (readerSettings.width / 100), margin: "0 auto" }}>
        {chapter.pdfUrl ? (
...
function AnalyticsAdmin({ data, persist }) {
  ...
  // opportunistically trim analytics history older than 30 days so the doc doesn't grow forever
  useEffect(() => {
    const prunedDaily = pruneOldDaily(analytics.dailyViews);
    const prunedRef = pruneOldDaily(analytics.referrers);
    const changed = JSON.stringify(prunedDaily || {}) !== JSON.stringify(analytics.dailyViews || {}) ||
                     JSON.stringify(prunedRef || {}) !== JSON.stringify(analytics.referrers || {});
    if (changed) {
      persist({ ...data, analytics: { ...analytics, dailyViews: prunedDaily || {}, referrers: prunedRef || {} } });
    }
  }, []);
  ...
  const last14 = Array.from({ length: 14 }).map((_, i) => {
    ...
    return { key, label: d.toLocaleDateString(undefined, { month: "short", day: "numeric" }), views: dailyViews[key] || 0 };
  });
  const maxDaily = Math.max(1, ...last14.map(d => d.views));
  const thisWeek = last14.slice(7).reduce((a, d) => a + d.views, 0);
  const lastWeek = last14.slice(0, 7).reduce((a, d) => a + d.views, 0);
  const weekChangePct = lastWeek > 0 ? Math.round(((thisWeek - lastWeek) / lastWeek) * 100) : (thisWeek > 0 ? 100 : 0);

  const referrers = analytics.referrers || {};
  const sourceTotals = {};
  Object.values(referrers).forEach(dayObj => {
    Object.entries(dayObj || {}).forEach(([src, n]) => { sourceTotals[src] = (sourceTotals[src] || 0) + n; });
  });
  const topSources = Object.entries(sourceTotals).sort((a, b) => b[1] - a[1]).slice(0, 6);
  const totalRefs = Object.values(sourceTotals).reduce((a, b) => a + b, 0);
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between" style={{ marginBottom: 12 }}>
        <p className="mono" style={{ color: "var(--muted)", fontSize: 12, textTransform: "uppercase" }}>Views — last 14 days</p>
        {(thisWeek > 0 || lastWeek > 0) && (
          <span className="mono" style={{ fontSize: 12, color: weekChangePct >= 0 ? "var(--jade)" : "var(--accent)" }}>
            {weekChangePct >= 0 ? "▲" : "▼"} {Math.abs(weekChangePct)}% vs prior week
          </span>
        )}
      </div>
      <div className="flex items-end" style={{ gap: 4, height: 90, marginBottom: 8, padding: "0 2px" }}>
        {last14.map(d => (
          <div key={d.key} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "flex-end", height: "100%" }} title={`${d.label}: ${d.views} views`}>
            <div style={{ width: "100%", maxWidth: 22, height: `${Math.max(3, (d.views / maxDaily) * 100)}%`, background: "var(--accent)", borderRadius: "3px 3px 0 0" }} />
          </div>
        ))}
      </div>
      <div className="flex" style={{ gap: 4, marginBottom: 28, padding: "0 2px" }}>
        {last14.map((d, i) => (
          <span key={d.key} className="mono" style={{ flex: 1, textAlign: "center", fontSize: 9, color: "var(--muted)", display: i % 2 === 0 ? "block" : "none" }}>{d.label}</span>
        ))}
      </div>

      <p className="mono" style={{ color: "var(--muted)", fontSize: 12, marginBottom: 12, textTransform: "uppercase" }}>Traffic sources</p>
      {topSources.length === 0 ? (
        <p style={{ color: "var(--muted)", fontSize: 14, marginBottom: 28 }}>No referrer data recorded yet.</p>
      ) : (
        <div style={{ marginBottom: 28 }}>
          {topSources.map(([src, n]) => (
            <div key={src} style={{ padding: "8px 0", borderBottom: "1px solid var(--hairline)" }}>
              <div className="flex items-center justify-between" style={{ marginBottom: 5 }}>
                <span style={{ fontSize: 13, textTransform: "capitalize" }}>{src}</span>
                <span className="mono" style={{ fontSize: 12, color: "var(--muted)" }}>{n} · {totalRefs ? Math.round((n / totalRefs) * 100) : 0}%</span>
              </div>
              <div style={{ height: 4, background: "var(--ink)", borderRadius: 2 }}>
                <div style={{ height: "100%", width: `${totalRefs ? (n / totalRefs) * 100 : 0}%`, background: "var(--jade)", borderRadius: 2 }} />
              </div>
            </div>
          ))}
        </div>
...
function ChaptersAdmin({ data, persist }) {
  ...
  const [pastePages, setPastePages] = useState("");
  const [pdfUrl, setPdfUrl] = useState("");
  const [contentType, setContentType] = useState("images"); // "images" | "pdf"
  const [status, setStatus] = useState("published");
  const [scheduleEnabled, setScheduleEnabled] = useState(false);
  const [publishAtStr, setPublishAtStr] = useState("");
  const [editingId, setEditingId] = useState(null);
  ...
  const resetForm = () => {
    setNumber(""); setTitle(""); setDateStr(""); setPages([]); setPastePages(""); setPdfUrl("");
    setContentType("images"); setStatus("published"); setScheduleEnabled(false); setPublishAtStr(""); setEditingId(null);
  };
  ...
  const startEdit = (c) => {
    ...
    if (c.publishAt) {
      ...
      setPublishAtStr(`${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}T${pad(d.getHours())}:${pad(d.getMinutes())}`);
    } else {
      setScheduleEnabled(false);
      setPublishAtStr("");
    }
    if (c.pdfUrl) { setContentType("pdf"); setPdfUrl(c.pdfUrl); setPages([]); }
    else { setContentType("images"); setPages(c.pages || []); setPdfUrl(""); }
    setPastePages("");
    window.scrollTo({ top: 0, behavior: "smooth" });
  };

  const addPastedPages = () => {
    const urls = pastePages.split("\n").map(l => l.trim()).filter(Boolean);
    if (urls.length) setPages(prev => [...prev, ...urls]);
    setPastePages("");
  };
  ...
  const saveChapter = () => {
    ...
    const finalPages = contentType === "images" ? pages : [];
    const publishAt = scheduleEnabled && publishAtStr ? new Date(publishAtStr).getTime() : null;
    if (editingId) {
      const next = { ...data, chapters: data.chapters.map(c => c.id === editingId
        ? { ...c, seriesId, number: Number(number), title: title.trim(), date: dateStr.trim(), pages: finalPages, pdfUrl: chapterPdfUrl, status, publishAt }
        : c) };
      persist(next);
    } else {
      const next = { ...data, chapters: [...data.chapters, { id: uid("ch"), seriesId, number: Number(number), title: title.trim(), date: dateStr.trim(), pages: finalPages, pdfUrl: chapterPdfUrl, status, publishAt, createdAt: Date.now() }] };
      persist(next);
    }
    resetForm();
  };
  ...
  return (
    <div>
      <div style={{ background: "var(--surface)", border: "1px solid var(--hairline)", borderRadius: 10, padding: 20, marginBottom: 28 }}>
        ...
        <label style={labelStyle}>Publish date (optional, display text)</label>
        ...
        {contentType === "images" ? (
          <>
            <label style={labelStyle}>Page images — in reading order (upload, or paste URLs below)</label>
            ...
            {pages.length > 0 && (
              <div style={{ marginTop: 10, marginBottom: 12 }}>
                {pages.map((url, idx) => (
                  <div key={idx} className="flex items-center" style={{ gap: 10, padding: "6px 0", borderBottom: "1px solid var(--hairline)" }}>
                    <div style={{ width: 40, height: 40, borderRadius: 5, overflow: "hidden", background: "var(--ink)", flexShrink: 0 }}>
                      <img src={url} alt={`Page ${idx + 1}`} style={{ width: "100%", height: "100%", objectFit: "cover" }} onError={(e) => { e.target.style.display = "none"; }} />
                    </div>
                    <span className="mono" style={{ fontSize: 11, color: "var(--muted)", width: 20, flexShrink: 0 }}>{idx + 1}</span>
                    <span style={{ flex: 1, fontSize: 11, color: "var(--muted)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{url}</span>
                    <button type="button" onClick={() => movePage(idx, -1)} disabled={idx === 0} style={{ background: "none", border: "none", color: idx === 0 ? "var(--hairline)" : "var(--paper)", cursor: idx === 0 ? "default" : "pointer" }}><ArrowUp size={14} /></button>
                    <button type="button" onClick={() => movePage(idx, 1)} disabled={idx === pages.length - 1} style={{ background: "none", border: "none", color: idx === pages.length - 1 ? "var(--hairline)" : "var(--paper)", cursor: idx === pages.length - 1 ? "default" : "pointer" }}><ArrowDown size={14} /></button>
                    <button type="button" onClick={() => removePage(idx)} style={{ background: "none", border: "none", color: "var(--muted)", cursor: "pointer" }}><X size={14} /></button>
                  </div>
                ))}
              </div>
            )}

            <label style={labelStyle}>Paste more page URLs (one per line, added to the end)</label>
            <textarea style={{ ...inputStyle, minHeight: 70, resize: "vertical", fontFamily: "'IBM Plex Mono', monospace", fontSize: 12 }}
              value={pastePages} onChange={e => setPastePages(e.target.value)} placeholder={"https://example.com/page1.jpg\nhttps://example.com/page2.jpg"} />
            <button type="button" onClick={addPastedPages} style={{ background: "none", border: "1px solid var(--hairline)", color: "var(--paper)", padding: "7px 14px", borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: "pointer", marginBottom: 12 }}>Add pasted pages</button>
          </>
        ) : (
          <>
        ...
        {status === "published" && (
          <div style={{ marginBottom: 12 }}>
            <label className="flex items-center" style={{ gap: 8, fontSize: 13, color: "var(--paper)", cursor: "pointer", marginBottom: scheduleEnabled ? 8 : 0 }}>
              <input type="checkbox" checked={scheduleEnabled} onChange={e => setScheduleEnabled(e.target.checked)} />
              Schedule for later instead of publishing immediately
            </label>
            {scheduleEnabled && (
              <input type="datetime-local" style={inputStyle} value={publishAtStr} onChange={e => setPublishAtStr(e.target.value)} />
            )}
          </div>
        )}
      ...
      {chaptersForSeries.map(c => {
        const isScheduled = c.status !== "draft" && c.publishAt && c.publishAt > now;
        return (
        <div key={c.id} className="flex items-center justify-between" style={{ padding: "12px 0", borderBottom: "1px solid var(--hairline)" }}>
          <div>
            <div className="flex items-center" style={{ gap: 8 }}>
              ...
              {isScheduled && (
                <span className="mono" style={{ fontSize: 10, color: "var(--ink)", background: "var(--jade)", borderRadius: 999, padding: "2px 8px" }}>
                  SCHEDULED · {new Date(c.publishAt).toLocaleString(undefined, { month: "short", day: "numeric", hour: "numeric", minute: "2-digit" })}
                </span>
              )}
            </div>
