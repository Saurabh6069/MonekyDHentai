...
import { BookOpen, Plus, Trash2, ChevronLeft, ChevronRight, Lock, X, ScrollText, Menu, LogOut, Pencil, Search, Star, Bookmark, BookmarkCheck, Share2, Eye, ArrowUp, ArrowDown, MessageCircle, Upload, Download, Maximize2, Minimize2 } from "lucide-react";
...
export default function WebtoonSite() {
  ...
  // Keep our fullscreen flag in sync if the reader exits fullscreen via Esc,
  // a back gesture, or any other browser-native way (not just our own button).
  useEffect(() => {
    const onFsChange = () => {
      const fsEl = document.fullscreenElement || document.webkitFullscreenElement;
      if (!fsEl) setReaderFullscreen(false);
    };
  ...
  // Leaving the reader entirely should also drop out of reading mode.
  useEffect(() => {
    if (view !== "reader" && readerFullscreen) setReaderFullscreen(false);
  }, [view]);
  ...
  return (
    <div style={{ background: "var(--ink)", color: "var(--paper)", minHeight: "100vh", fontFamily: "'Noto Sans KR', sans-serif" }}>
      ...
      {!(view === "reader" && readerFullscreen) && (
        <TopBar data={data} onHome={goHome} onAdmin={goAdmin} view={view} menuOpen={menuOpen} setMenuOpen={setMenuOpen} />
      )}
      ...
      {!loaded ? (
        ...
        <Reader data={data} seriesId={seriesId} chapterId={chapterId} onOpenChapter={openChapter} onBackToSeries={backToSeries}
                isFullscreen={readerFullscreen} setIsFullscreen={setReaderFullscreen} />
      ) : (
...
function Reader({ data, seriesId, chapterId, onOpenChapter, onBackToSeries, isFullscreen, setIsFullscreen }) {
  ...
  const toggleFullscreen = async () => {
    ...
    try {
      ...
      // Fullscreen API unsupported or blocked (common on iOS Safari) — the
      // distraction-free layout below still applies even without real
      // device fullscreen, so reading mode still works.
    }
  ...
  return (
    <div ref={containerRef} style={{ position: "relative" }}>
      ...
      {!isFullscreen && (
        <div className="flex items-center justify-between" style={{ maxWidth: 780, margin: "0 auto", padding: "24px 24px 0" }}>
          ...
          <div className="flex items-center" style={{ gap: 14 }}>
            ...
            <button onClick={toggleFullscreen} title="Fullscreen reading mode" className="mono" style={{ background: "none", border: "none", color: "var(--muted)", cursor: "pointer", fontSize: 12, display: "flex", alignItems: "center", gap: 5 }}>
              <Maximize2 size={14} />
            </button>
      ...
      {isFullscreen && (
        <button onClick={toggleFullscreen} title="Exit fullscreen"
                style={{
                  ...
                  background: "rgba(20,19,27,0.75)", backdropFilter: "blur(6px)",
      ...
      <div style={{ maxWidth: 720, margin: "0 auto", paddingTop: isFullscreen ? 24 : 0 }}>
        {chapter.pdfUrl ? (
